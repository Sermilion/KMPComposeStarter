package news.readian.notoesapp.codegen
...
class ViewModelInjectProcessor(
  ...
  private fun generateViewModelEntry(classDeclaration: KSClassDeclaration, resolver: Resolver) {
    ...
    val scopeClassName = classDeclaration.annotations.firstOrNull { annotation ->
      ...
      when (scopeValue) {
        "news.readian.notoesapp.common.di.AppScope",
        "software.amazon.lastmile.kotlin.inject.anvil.AppScope",
        ->
          ClassName("software.amazon.lastmile.kotlin.inject.anvil", "AppScope")
        "news.readian.notoesapp.common.di.UserScope" ->
          ClassName("news.readian.notoesapp.common.di", "UserScope")
        "news.readian.notoesapp.common.di.ScreenScope" ->
          ClassName("news.readian.notoesapp.common.di", "ScreenScope")
        else -> {
    ...
    val viewModelEntry = ClassName("news.readian.notoesapp.common.di", "ViewModelEntry")
    val viewModelClass = ClassName("androidx.lifecycle", "ViewModel")
    val assistedArgs = ClassName("news.readian.notoesapp.common.di", "AssistedArgs")
